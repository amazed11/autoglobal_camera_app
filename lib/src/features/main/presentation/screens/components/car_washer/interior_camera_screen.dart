...
class InteriorCameraScreen extends StatefulWidget {
  const InteriorCameraScreen({super.key});

  @override
  State<InteriorCameraScreen> createState() => InteriorCameraScreenState();
}

class InteriorCameraScreenState extends State<InteriorCameraScreen> {
  ...
  void takePicture() async {
    if ((getIt<CameraCubit>().state.interiorPhotos?.length ?? 0) >= 10) {
      errorToast(msg: "You reached limit for maximum photos i.e. 10/10");
      return;
    }
    XFile? image;
    ...
    return Scaffold(
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          ...
          if (((getIt<CameraCubit>().state.interiorPhotos?.length ?? 0) +
                  (imagesList.length)) >=
              10) {
            errorToast(msg: "You reached limit for maximum photos i.e. 10/10");
            return;
                    ...
                    GestureDetector(
                      onTap: () {
                        if (imagesList.isNotEmpty) {
                          getIt<CameraCubit>().addInteriorPhotos(imagesList);
                          context.pop();
                        } else {
