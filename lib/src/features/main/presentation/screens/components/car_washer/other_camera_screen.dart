...
class OtherCameraScreen extends StatefulWidget {
  const OtherCameraScreen({super.key});

  @override
  State<OtherCameraScreen> createState() => OtherCameraScreenState();
}

class OtherCameraScreenState extends State<OtherCameraScreen> {
  ...
  void takePicture() async {
    if ((getIt<CameraCubit>().state.otherPhotos?.length ?? 0) >= 5) {
      errorToast(msg: "You reached limit for maximum photos i.e. 5/5");
      return;
    }
    XFile? image;
    ...
    return Scaffold(
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          ...
          if (((getIt<CameraCubit>().state.otherPhotos?.length ?? 0) +
                  (imagesList.length)) >=
              5) {
            errorToast(msg: "You reached limit for maximum photos i.e. 5/5");
            return;
                    ...
                    GestureDetector(
                      onTap: () {
                        if (imagesList.isNotEmpty) {
                          getIt<CameraCubit>().addOtherPhotos(imagesList);
                          context.pop();
                        } else {
